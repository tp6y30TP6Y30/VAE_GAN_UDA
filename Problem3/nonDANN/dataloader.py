...
class dataloader(Dataset):
	def __init__(self, mode, img_path, folder, usps):
		...
		self.transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize([0.5], [0.5])]) if self.usps else transforms.Compose([transforms.ToTensor(), transforms.Grayscale(3), transforms.Normalize([0.5], [0.5])])
